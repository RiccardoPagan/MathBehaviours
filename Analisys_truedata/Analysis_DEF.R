...
Path <- getwd()
d <- read.csv("DatasetBehaviours.csv", sep = ",")
...
fit_cat <- brm(
  ...
  data = d, family = categorical(link = "logit"), cores = 4, warmup = 1000, iter = 4000
  #backend = "cmdstanr", threads = threading(2)
)
fit_ord <- brm(
  choice_ord ~ buoyancy + avoidance + worry + sc_state + math_anxiety + sc_trait + math_ability_T0 + Gender + (1|id),
  data = d, family = cumulative(link = "logit"), cores = 4, warmup = 1000, iter = 4000
  #backend = "cmdstanr", threads = threading(2)
)
...
d_buoy <- d %>%
  ...
  select(buoy1 = trait_buoy_1, buoy2 = trait_buoy_2,
         buoy3 = trait_buoy_3, buoy4 = trait_buoy_4)

d_av <- d %>%
  distinct(id, .keep_all = TRUE) %>%
  select(av1 = trait_avoidance_1, av2 = trait_avoidance_2, av3 = trait_avoidance_3,
         av4 = trait_avoidance_4, av5 = trait_avoidance_5, av6 = trait_avoidance_6,
         av7 = trait_avoidance_7, av8 = trait_avoidance_8, av9 = trait_avoidance_9,
         av10 = trait_avoidance_10)
...
summary(fitAv, fit.measures = T, standardized = T)


# looking at fit indices including decisional rules
(fm = fitMeasures(fitBuoy, fit.measures=c("CFI","RMSEA")))
FitBuoy_ok = fm["cfi"]>0.95 & fm["rmsea"]<0.08
if(FitBuoy_ok) print("Fit indices ok: proceed!") else print("Stop there!")

(fm = fitMeasures(fitAv, fit.measures=c("CFI","RMSEA")))
FitAv_ok = fm["cfi"]>0.95 & fm["rmsea"]<0.08
if(FitAv_ok) print("Fit indices ok: proceed!") else print("Stop there!")

# 2. Checking reliability of latent factor
(rel = semTools::reliability(fitBuoy))
RelBuoy_ok = rel["alpha.ord",] > 0.50
if(RelBuoy_ok) print("Reliability ok: proceed!") else print("Stop there!")

(rel = semTools::reliability(fitAv))
RelAv_ok = rel["alpha.ord",] > 0.50
if(RelAv_ok) print("Reliability ok: proceed!") else print("Stop there!")
...
# path "a" — trait -> state
# MODIFICATION 1: previouly we were estimating sigma within a subject 
# with 32 identical rows -> chains do not converge
# so in both bf_worry e bf_sctstate are estimated with only one row
#MODIFICATION 2: with one measure for each subject random intercept (1|id) was removed
d$first_trial = as.numeric(!duplicated(d$id))
bf_worry   <- bf(worry | subset (first_trial) ~ math_anxiety + math_ability_T0 + Gender)
bf_scstate <- bf(sc_state | subset (first_trial) ~ sc_trait + math_ability_T0 + Gender)
...
# model comparison
loo_full <- loo(fit_full, resp = "choicec")
loo_partial <- loo(fit_partial, resp = "choicec")
...
# Path "b"
if (model_type == "ordinal") {
  b1 <- draws_choice$b_choiceord_worry
  b2 <- draws_choice$b_choiceord_sc_state
} else {
  b1 <- draws_choice$b_muself_choicec_worry
  b2 <- draws_choice$b_muself_choicec_sc_state
}
...
if (model_type == "ordinal") {
  ...
  h_worry_self    <- hypothesis(best_choice_model, "muself_choicec_worry < 0")
  h_sc_state_self <- hypothesis(best_choice_model, "muself_choicec_sc_state > 0")
  h_worry_help    <- hypothesis(best_choice_model, "muhelp_choicec_worry < 0")
  h_sc_state_help <- hypothesis(best_choice_model, "muhelp_choicec_sc_state > 0")
  ...
  if (FitBuoy_ok & RelBuoy_ok) {
    h_buoyancy_self <- hypothesis(best_choice_model, "muself_choicec_buoyancy > 0")
    h_buoyancy_help <- hypothesis(best_choice_model, "muhelp_choicec_buoyancy > 0")
  ...
  if (FitAv_ok & RelAv_ok) {
    h_avoidance_self <- hypothesis(best_choice_model, "muself_choicec_avoidance < 0")
    h_avoidance_help <- hypothesis(best_choice_model, "muhelp_choicec_avoidance < 0")
  ...
  h_diff <- hypothesis(best_choice_model,
                       c("muself_choicec_worry < muhelp_choicec_worry",
                         "muself_choicec_sc_state > muhelp_choicec_sc_state"))
  print(h_diff)
}

write.excel (results_indirect, "Results_indirect_old.xlsx")
write.excel (h_diff, "Results_Model_old.xlsx")
